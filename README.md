...
**One-click entry:** [**unstable_bridge → ablate node 0**](https://henry-filgueiras.github.io/resonant-instrument-lab/demo/index.html?atlas=../runs/demo/unstable_bridge/atlas.json&select=ablate_n0). Removing the bridge node flips three detectors simultaneously (`flam → silent`, `phase_beating → FIRED`, `polyrhythmic → FIRED`) and the pulse raster audibly dissolves — press **play B** under the comparison card to hear it.

Landing page, with the rest of the catalogue:

> **https://henry-filgueiras.github.io/resonant-instrument-lab/**

No backend, no WASM simulator, no model in the loop — the browser fetches the same `summary.json` / `atlas.json` / `audio.wav` artifacts the Python simulator produces at build time. The Pages tree is regenerated by `python scripts/build_pages.py` and committed under `docs/`.
...
`./run.sh` creates `.venv`, regenerates every fixture's artifacts (including `atlas.json`), prints copy-pasteable browser URLs, and serves them on `http://localhost:8000`. Deep-link to any intervention with `&select=<id>` (e.g. `&select=ablate_n4`).

Eight regime fixtures run end-to-end. Each produces a full `state.npz` / `events.jsonl` / `topology.json` / `audio.wav` / frozen `config.yaml` bundle, plus — with `--summary` — a compact semantic verdict from the current detectors:
...
Swap the config to move through the detector matrix: `regime_drifting` fires `drifting`; `regime_two_cluster` fires `dominant_cluster` on a clean 4+4 velocity split (tail 2-way separability ~700); `regime_phase_beating` fires `phase_beating` on a near-frequency pair in an incoherent field; `regime_flam` fires `flam` on a frequency-locked near-unison pair; `regime_polyrhythmic` fires `polyrhythmic` on a 2:3 pulse-rate ratio; `regime_unstable_bridge` fires the first counterfactual detector (`sim.ablate.ablate_node` run on every cluster member, flagging nodes whose removal collapses surviving-member `local_r` below 0.9); `regime_brittle_lock` fires the second (`sim.ablate.nudge_node` run on every node, flagging nodes whose +0.25 Hz nudge drops tail-r below 0.9). Thresholds and windows live inline in `sim/detectors.py`; confidence is evidence margin, not a probability.
